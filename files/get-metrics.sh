...
#RESTAPI_PORT=805
#DBSYNC_PROM_HOST=127.0.0.1
#DBSYNC_PROM_PORT=8080
#PGDATABASE=cexplorer
#PGOPTIONS='--statement_timeout=1s'
#PGUSER=postgrest
#NODE_PROM_URL="http://172.16.100.112:12798/metrics"
...
[[ -z ${PGUSER} ]] && PGUSER=postgrest
[[ -z ${NODE_PROM_URL} ]] && NODE_PROM_URL="http://127.0.0.1:12798/metrics"
...
function get_metrics() {
  ...
  currslottip=$(echo "4492800 + (`date +"%s"` - 1596059091) / 1 - 1" | bc)
  ...
  # in Bytes
  pubschsize=$(psql -t --csv -d ${PGDATABASE} -U $PGUSER -c "SELECT sum(pg_total_relation_size(quote_ident(schemaname) || '.' || quote_ident(tablename))::bigint) FROM pg_tables WHERE schemaname = 'public'" | grep "^[0-9]")
  grestschsize=$(psql -t --csv -d ${PGDATABASE} -U $PGUSER -c "SELECT sum(pg_total_relation_size(quote_ident(schemaname) || '.' || quote_ident(tablename))::bigint) FROM pg_tables WHERE schemaname = 'grest'" | grep "^[0-9]")
  grestconns=$(psql -t --csv -d ${PGDATABASE} -U $PGUSER -c "select count(1) from pg_stat_activity where state='active' or state='idle';" | awk '{print $1}')
  dbsize=$(psql -t --csv -d ${PGDATABASE} -U $PGUSER -c "SELECT pg_database_size ('${PGDATABASE}');" | grep "^[0-9]")
  # Metrics
  export METRIC_dbsynctipref=$(( currslottip - $( echo "${tip}" | jq .[0].abs_slot) ))
  export METRIC_nodetipref=$(( currslottip - slotnum ))
  export METRIC_uptime=$(ps -o etimes -p `pidof cardano-node` | tail -n 1 | xargs)
  export METRIC_dbsyncBlockHeight=$(echo "${tip}" | jq .[0].block_no)
  export METRIC_nodeBlockHeight=$(curl -s $NODE_PROM_URL | grep blockNum | awk '{print $2}')
